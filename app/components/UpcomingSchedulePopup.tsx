...
export default function UpcomingSchedulePopup() {
  ...
  useEffect(() => {
    const handleTogglePopup = () => {
      setDismissed((prev) => {
        if (prev) {
          // Se estava fechado (dismissed), abre.
          setExpanded(true);
          return false;
        } else {
          // Se estava aberto, apenas fecha (dismiss).
          return true;
        }
      });
    };

    window.addEventListener("open-schedule-popup", handleTogglePopup);
    return () => window.removeEventListener("open-schedule-popup", handleTogglePopup);
  }, []);
            ...
            <ul className="space-y-3">
              {items.map(({ event, date }) => {
                ...
                return (
                  <li
                    key={event.id}
                    className="rounded-[20px] border border-slate-100 bg-slate-50 px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-900"
                  >
